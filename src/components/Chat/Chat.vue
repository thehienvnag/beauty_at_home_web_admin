    ...
    <div :class="{'chatPanel chatContacts': true, chatMessageOpen: chatMessageOpened}">
      ...
      <b-list-group id="chat-sidebar-user-group" class="chatSidebarUserGroup">
        <b-list-group-item
          ...
          :class="{active: index === 0 && chatNotificationMessageState === 'NEW'}"
        >
          <i :class="`fa fa-circle float-right text-${conversation.status}`" />
          <span v-if="index === 0 && chatNotificationMessageState === 'NEW'" class="badge badge-danger badge-pill float-right animated bounceInDown">3</span>
